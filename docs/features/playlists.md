...
For rule-based playlists, see [smart-playlists.md](smart-playlists.md). This doc covers manually-curated playlists: creating, viewing/playing, adding/removing tracks, custom cover art, deleting.
...
- `src/features/playlists/usePlaylists.ts:42-52` — `createPlaylist`
- `src/features/playlists/usePlaylists.ts:54-62` — `deletePlaylist` (deletes server + `playlist_tracks` + `playlists` rows)
- `src/features/playlists/usePlaylists.ts:64-89` — `addTrackToPlaylist`
- `src/features/playlists/usePlaylists.ts:91-106` — `renamePlaylist` (name + comment)
- `src/features/playlists/usePlaylists.ts:108-140` — `addAlbumToPlaylist` (adds every track on an album, ordered by disc/track number)
- `src/features/playlists/usePlaylists.ts:142-146` — `setCustomCover`
- `src/features/playlists/usePlaylistTracks.ts:33-52` — `removeTrack(position, playlist, swc)` — calls `removeTrackFromNavidromePlaylist` by position index, then deletes local row + decrements `track_count`
- `src/features/playlists/usePlaylistTracks.ts:13-31` — track list query, joined `playlist_tracks` + `tracks` + `albums`, ordered by `position`
