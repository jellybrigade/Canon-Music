...
- **Query hook**: `useSearch(query)` in `src/features/search/useSearch.ts:131-139` — thin `useQuery` wrapper, `enabled` only when trimmed query is non-empty.
...
- **Results UI**: `src/features/search/SearchResults.tsx` — grouped sections, per-group "show all" state (`showAllArtists/Albums/Tracks`), context menus via shared `ContextMenu` + `StartRadioSubmenu` components.
- **React Query key**: `QK.search(trimmed)` — `src/lib/queryKeys.ts:24`.
...
- **Styling**: `src/app/library.css:278-356` (search bar/trigger button), `src/features/search/SearchResults.css` (grouped result rows, thumbnails, show-all buttons).
