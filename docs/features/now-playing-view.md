...
- `src/features/playback/components/NowPlayingView.tsx:155` — main component. `Tab` type = `"up-next" | "about" | "lyrics"` (line 31).
...
- Lyrics: `src/features/playback/hooks/useLyrics.ts:26` `useLyrics()` (LRClib → Navidrome → lyrics.ovh fallback chain), `:54` (manual override persisted to SQLite `lyrics` table), `src/clients/lrclib.ts:43` `parseLrc()`. Called at `NowPlayingView.tsx:192`; auto-scroll at `:288`; seek-on-click at `:317`.
