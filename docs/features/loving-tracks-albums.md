...
- **Player bar** (`src/features/playback/components/PlayerBar.tsx:394,527`) — heart icon next to the currently-playing track, both compact and expanded layouts.
- **Now Playing view** (`src/features/playback/components/NowPlayingControls.tsx`) — heart button in the transport row, `now-playing-love-btn`.
- **Album detail page** (`src/pages/album/AlbumTrackList.tsx`) — heart icon on each track row (`track-heart`).
...
4. Loved albums appear in Home view's favorites row (`HomeView.tsx`) — this includes both albums loved directly and albums that are loved by virtue of having at least one loved track (`lovedTrackAlbumIds`).
...
- **Sync overwrites local loved state from server truth**: every library sync calls `getStarred2` and does a full `DELETE` + re-`INSERT` of `loved_albums`/`loved_tracks` for that server, replacing local state with whatever Navidrome reports as starred (`src/features/sync/sync.ts:204-226`). This runs independent of incremental sync skip logic, so it always executes on every sync pass.
...
- **React Query keys**: `QK.loved_tracks()`, `QK.loved_albums()`, `QK.loved_track_albums()` (`src/lib/queryKeys.ts`), all `staleTime: Infinity` (invalidated manually on toggle/sync, not polled).
- **Navidrome/Subsonic calls**: `starTrack`/`unstarTrack`/`starAlbum`/`unstarAlbum` in `src/clients/navidrome.ts`, thin wrappers over `star.view`/`unstar.view`.
- **Sync**: `fetchStarred2` (`getStarred2` Subsonic call) + full local overwrite in `src/features/sync/sync.ts:204-226`; invalidated post-sync in `src/features/sync/useLibrarySync.ts:45-46`.
- **Radio**: "Loved" is one of Radio's 8 modes — queries `loved_tracks` directly (`src/features/radio/lib/radio.ts:306-311`, `src/features/playback/store/player.ts:36-46,1362`); see [radio-auto-dj.md](radio-auto-dj.md).
- **Other UI surfaces**: Artist detail "Favorites" section filters an artist's top tracks down to loved ones (`src/pages/ArtistDetail.tsx:104,118-121,405-427`); Home view has a dedicated "Loved" spotlight carousel (`src/pages/HomeView.tsx`); Now Playing's Up Next queue rows show a loved indicator (`UpNextList.tsx`, `now-playing-up-next-loved`).
