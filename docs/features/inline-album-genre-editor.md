...
- Both actions call `normalizeAlbum()` (`src/features/tags/lib/tagNormalize.ts`) then invalidate React Query keys `QK.normalizedTags(albumId)` and (for exclude) `QK.albumGenreExclusions(albumId)`.
- Unmatched resolver: `UnmatchedSection` in `src/features/tags/components/TagDrawer.tsx:176-253`, backed by `useAlbumUnmatchedGenres` (`TagDrawer.tsx:74`, query key `QK.albumUnmatchedGenres`) and `useTagMappings().saveMapping` (`src/features/tags/hooks/useTagMappings.ts:29`) which writes to `tag_mappings`. Accept/Ignore pass sentinel canonical IDs `ACCEPTED`/`IGNORED`; picking a tree node passes its canonical ID via `CanonCombobox`.
- Autocomplete source: `getCanonTree()` from `src/features/tags/lib/canonicalize.ts`, filtered client-side.
