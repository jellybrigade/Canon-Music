...
- **Types & state**: `src/features/playback/store/player.ts:29-37` (`RadioMode` union), `:39-48` (`RADIO_MODES` — mode/label pairs, single source of truth for all UI strings), `:154-158` (`radioActive`, `radioSeed`, `radioMode`, `radioLabel`, `radioSimilarityScale` state).
...
- **Candidate scoring**: `src/features/radio/lib/radio.ts`
...
- **Queue-filling hook**: `src/features/radio/hooks/useRadio.ts`
...
- **UI**: `src/components/RadioChip.tsx` + `RadioChip.css` (persistent chip + dropdown), `src/features/radio/components/StartRadioSubmenu.tsx` (shared context-menu submenu component)
