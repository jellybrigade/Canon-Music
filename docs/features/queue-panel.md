...
- **Player bar → "Queue" button** (list icon, right side of player bar, `src/features/playback/components/PlayerBar.tsx:464-471`) — toggles a resizable side panel titled `Queue (N)`.
...
- **Trigger**: `src/features/playback/components/PlayerBar.tsx:464-471` (`toggleQueue()`).
- **Zustand store** (`src/features/playback/store/player.ts`):
