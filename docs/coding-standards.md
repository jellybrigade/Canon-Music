...
Ref impl: `src/features/tags/components/TagsView.css` (`.tags-tab-btn`).
...
- File names: components PascalCase, hooks `useX`, everything else camelCase (`queryKeys.ts`,
  not `query-keys.ts`). A test names its module first, then any aspect: `player.waste.test.ts`.
  Enforced by `src/lib/fileNaming.test.ts`
...
- One or two lines, not an essay. Deep rationale goes to `known-issues.md` or
  `docs/ARCHITECTURE.md`, which are greppable; a comment is not
...
- Subscribe to the narrowest store slice the component reads. A component re-rendering
  on a slice it does not read is a defect with a waste test attached (see AGENTS.md)
...
- Schema changes are a numbered migration block, never an in-place edit of an old one.
  New tables and columns get an `docs/ARCHITECTURE.md` line in the same commit
...
`src/store` shared browse state, `src/db` schema and queries. Code one feature owns lives in
`src/features/<name>/` (flat under ~15 files, else the same `components/ hooks/ lib/ store/`
split), and other code imports from it there. A pure function extracted out of
