...
- Bug classes, greps and generalizations go to `docs/known-issues.md`, versioned
...
5. `docs/ARCHITECTURE.md` updated if files, data flow, invariants, Tauri commands
   or migrations changed
6. New bug fixed -> `docs/known-issues.md` entry **and** its regression test are
   in this same commit
7. `git status` clean of unrelated files. **Nothing under `instructions/` is staged** -
   that directory is gitignored; versioned docs live in `docs/`
