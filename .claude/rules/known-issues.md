...
- **A cache-hit test written as "is there a value" never caches the answer "there is none", which is the case that repeats.** `useLyrics` treated a row with null `plain`/`synced` as a miss so `refresh` could force a re-lookup, but the queryFn writes exactly that row when every source comes back empty - so every track with no lyrics anywhere re-ran the OpenSubsonic call, LRClib and lyrics.ovh on every open of the tab, forever. `source` now carries a `"cleared"` sentinel written by `refresh` and by the offset-only insert, and anything else is a completed lookup. Both columns are NOT NULL, so absence could not be the witness. Ask of any cache: what does a successful "nothing found" look like in the table, and does the hit test recognise it?
  ...
  ```
- **A query key written inline is a key nothing else can name, so nothing else can invalidate it.** `AlbumDetailRoute` held `["album-by-id", albumId, serverId]` inline in `AppRoutes.tsx`, the only occurrence of that string in `src/`, and the album page derives its accent color by decoding the cover and caching it on the row that query returns. The `UPDATE` had no invalidation, so for the 60s default `staleTime` (`src/main.tsx:30`, with `refetchOnWindowFocus: false`) the cached row still read `accent_color = null` and every revisit inside that window decoded the cover again to recompute a color already in the database. Second instance of exactly this: the comment on `PlaylistDetailRoute` records `["playlist-by-id"]` dying of the same cause, which is why that route resolves out of the `playlists` list instead - a remedy *not* open to albums, because `useAlbums` is skipped off `/library` (`App.tsx:175`) and filtered by `canonicalIdFilters`, so the list is neither complete nor present on the album route. The key is now `QK.albumById`, nested under `albums` so a broad `QK.albumsAll()` invalidation reaches it, and the two writers of `albums.accent_color` (the album page and the home spotlight) share `useAlbumAccent`, which persists *and* invalidates. The duplication was already rotting in the usual way: only one of the two copies had a `.catch`. `ArtistDetailRoute`'s `["artist-by-name"]` was the same shape and moved to `QK.artistByName` in the same pass - latent rather than live, because the two enrichment columns it selects (`lastfm_image_url`, `wikidata_image_url`) are read by nothing: `ArtistDetail` takes its portrait from `useEnrichArtist`'s own query, which *is* invalidated. Ask of any inline `queryKey`: who writes the row it selects, and can they name this key? The two remaining hits are MusicBrainz searches keyed on their own arguments with no local row behind them, so they are not instances; a third hit is.
  ```
...
- **A route that returns `null` for "don't know yet" and for "isn't there" paints the same blank page for both.** `data ?? null` collapses the one distinction `useQuery` gives you. Name the pending state.
  ...
  ```
- **A prerequisite gate is a state machine too, and a gate that fails closed with a *confident* message is worse than one that fails blank.** `if (!serverWithCred) return null` was a permanent blank page (`retry: false`). Shared `CredentialGate` in `AppRoutes.tsx`. `isPending` is useless on a gated query (disabled = pending forever); derive `!!server && !serverWithCred && !credError`. The fix covered the three detail routes only, and the grep below is written as an `if`, so it never saw the eight browse routes that gate on the same value as a JSX ternary. Those had drifted three ways: `/library` and `/artists` rendered "No server connected - add your Navidrome server in Settings", telling a user whose `servers` row is the reason the router mounted at all to go add the server they already have; `/playlists` rendered a bare "Loading…" that a failed read never left; `/home`, `/nowplaying`, `/years`, `/tracks` and `/unidentified` rendered an empty `<main>`. All eight now render the shared `CredentialNotice` (the gate's body, split out of `CredentialGate` so a route already inside its own `<main>` can use it), built once as `credentialNotice` in the `AppRoutes` body. The library header's own credential-error line went with it: it was driven by the same value as the gate beneath it, so the two could only ever appear together, and the page said it twice with two **Try again** buttons. There is deliberately still no "no server configured" branch anywhere - `App` renders the `Wizard` on an empty `servers` table. Ask of any prerequisite gate: how many states does the prerequisite have, does every branch of the gate name one of them, and is the *reassuring* branch reachable while the prerequisite is merely pending? Every hit below must resolve to `credentialNotice` or `CredentialGate`; the `enabled:` hits are query gates, not render gates.
  ```
