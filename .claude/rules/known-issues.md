...
## An identifier borrowed from an external service is compared exactly against a local one (fixed 2026-08-03)

`useSimilarInLibrary` decided which of Last.fm's similar artists the library owns with `SELECT name FROM artists WHERE name IN (...)` plus a `Set.has(name)` on the raw strings. Last.fm and Navidrome disagree on capitalisation constantly ("Tyler, The Creator" vs "Tyler, the Creator"), so an owned artist was filed under "Not in library". Aliases were ignored too, so an artist merged into a canonical name read as unowned.

**What made it more than a mislabelled tab:** the card's click handler navigates by that same borrowed string, and `ArtistDetailRoute` looked the artist up with an equally exact `WHERE a.name = ?`. That query misses for the same reason, and the route's fallback synthesises a minimal row from the URL - so the user landed on a real artist's page rendered as an empty one, with the album, track and genre queries downstream all matching on the wrong spelling. One comparison being exact produced a wrong badge; two of them in series produced a blank page.

**Fix:** both sides key on `LOWER(TRIM(...))`, the ownership query unions `artist_aliases`, and the route selects `a.name` so everything downstream queries with the library's own spelling rather than the external one. The hook still returns the caller's original strings, so its consumer's `set.has` stays an exact lookup.

**Generalizes:** any string that arrives from Last.fm, MusicBrainz, Wikidata or the server and is then matched against a local row is a join key you do not control the normalization of. Worth grepping for: `IN (${placeholders})` or `=` against `artists.name`, `tracks.title` or `albums.name` where the parameter came from a network response rather than from a previous local query. The tell that it matters is whether the same string is *also* used to navigate or to build a URL - a bad match that only mislabels is cosmetic, a bad match that then addresses a second lookup compounds. Note the fix has to reach every hop: normalizing the ownership check alone would have promoted the card to "in library" and still landed on the empty page.

**Second, unrelated trap in the same route, worth its own grep:** `useQuery`'s `data` is `undefined` while pending *and* when the query legitimately resolved to null, so `const row = data ?? fallback` renders the fallback during every load. Here that painted a real artist's hero as "0 albums in library" with no portrait until the query settled. Any `?? fallback` on a query result needs `isPending` checked first, or the fallback becomes the loading state by accident. In this case the gate also removed a remount, since the `key` was derived from the row that swapped underneath it.
