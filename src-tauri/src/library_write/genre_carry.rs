...
fn carry_genre_renames_in(
    ...
    mark_albums_for_renormalize(&tx, renames)?;
    ...
    let targets: HashMap<&str, &str> = renames
        .iter()
...
fn mark_albums_for_renormalize(tx: &Transaction, renames: &[GenreIdRename]) -> Result<(), String> {
    for rename in renames {
...
/// Renames in place and drops a duplicate the rename created. Returns whether anything changed.
fn rename_in_list(ids: &mut Vec<serde_json::Value>, targets: &HashMap<&str, &str>) -> bool {
