...
// trackIdTables.test.ts, which sweeps migrations.ts rather than trusting the next reader.
//
 ...
 * for ~87% of them). `tracks_fts` is not among them: `rebuildTracksFts` in src/features/sync/sync.ts rewrites
